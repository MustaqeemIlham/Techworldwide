...
$sql = "SELECT * FROM product WHERE Product_ID = $idProduct";
...
if (isset($_POST['submit'])) {
    ...
    $newQtty = $_POST['quantityBox'];
    $updatedQtty = $value['Product_Available'] - $newQtty;
    ...
    if (!$voucherflag) {
        ...
        if ($send) {
            $insert1 = "INSERT INTO payment(Payment_ID, Payment_Method, Payment_Date, Payment_Time, Payment_Charge, Voucher_ID, Purchase_ID,Payment_Status) VALUES ('','$method','$newdate','$time','$charge','$voucher',(SELECT Purchase_ID FROM purchase ORDER BY Purchase_ID DESC LIMIT 1), '$pstatus')";
            $send1 = mysqli_query($connect, $insert1);

            if ($send1) {
                $insert2 = "INSERT INTO detail(Purchase_ID, Product_ID, Quantity, Status, Delivery_Charge) VALUES ((SELECT Purchase_ID FROM purchase ORDER BY Purchase_ID DESC LIMIT 1), '$idProduct', '$newQtty', '$status', '$deliver')";
                $send2 = mysqli_query($connect, $insert2);
    ...
    <form action='' method='POST'>
        ...
        <div class="w3-third" style="padding-top: 80px;">
            ...
            <div class="icon-container style='margin-bottom:20px;margin-top:20px;'">
                <input type="radio" name="payType" id="online" value="COD" required onclick=" disablepay()"> Cash on Delivery &nbsp;&nbsp;&nbsp;
                <input type="radio" name="payType" id="card" value="Debit/Credit Card" required onclick=" disablepay()"> Debit/Credit Card
            </div><br><br>
            Name on Card<br>
        ...
        <div class="w3-third" style="padding-top: 80px;">
            ...
            <p>Name : <span><?php echo $value['Product_Name'] ?></span></p>
            <p>Price : RM <span><?php echo $value['Product_Price'] ?></span></p>
            <p>Quantity : <input type="text" name="quantityBox" id="quantitySpan" style="margin-left: 14.5%; width: 10%;text-align:center" readonly> </p>
            Sub-Total (RM):
        ...
        <input style=" width: 25%; margin-left: 67%; border-radius: 15px" type="submit" name="submit" value="Pay" class="btnCheckout">
    </form>
</div>
...
<script>
    let price = <?php echo $value['Product_Price'] ?>;
    let qtty = localStorage.getItem("inputValue");
    let newPrice = price * qtty;
    let charge = 0;
    let tot = newPrice;
    let voucValue = 0;
